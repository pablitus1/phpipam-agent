FROM php:5.6-apache
MAINTAINER Pierre Cheynier <pierre.cheynier@gmail.com> / Add gettext support by otto
...
RUN echo "00 * * * * /usr/local/bin/php /opt/phpipam-agent/index.php update > /proc/1/fd/1 2>/proc/1/fd/2" > ${CRONTAB_FILE} && \
    echo "10 * * * * /usr/local/bin/php /opt/phpipam-agent/index.php discover > /proc/1/fd/1 2>/proc/1/fd/2" > ${CRONTAB_FILE} && \
